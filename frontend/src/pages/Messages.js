...
const Messages = () => {
  ...
  const loadMessages = async (threadId, isPolling = false) => {
    try {
      const result = await messagingService.getMessages(threadId, { page_size: 50 });
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const result = await messagingService.replyToThread(selectedThread.id, newMessage);
  ...
  const startNewConversation = async () => {
    const contentValidation = messagingService.validateMessageContent(newConversation.content);
  ...
  const filteredThreads = threads.filter(thread => {
    ...
    return thread.last_message?.content?.toLowerCase().includes(query) ||
            ...
            {loading && threads.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {sortedThreads.map((thread) => {
                  const otherParticipant = messagingService.getOtherParticipant(thread, currentUser?.id);
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <p className={`text-sm truncate ${!thread.is_read ? 'font-semibold text-gray-900' : 'text-gray-700'}`}>
                              {otherParticipant?.username || 'Unknown User'}
                            </p>
                            <p className="text-xs text-gray-500 flex-shrink-0">
                              {messagingService.formatTimestamp(thread.last_message?.created_at || thread.created_at)}
                            </p>
                          </div>
                          ...
                          <p className="text-xs text-gray-500 truncate mt-1">
                            {messagingService.formatMessagePreview(thread.last_message, 40)}
                          </p>
                        </div>
                        {!thread.is_read && (
          ...
          {selectedThread ? (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="w-8 h-8 bg-gradient-to-r from-blue-400 to-purple-400 rounded-full flex items-center justify-center text-white text-sm font-medium">
                      {messagingService.getOtherParticipant(selectedThread, currentUser?.id)?.username?.charAt(0).toUpperCase() || 'U'}
                    </div>
                    <div>
                      <h2 className="text-lg font-semibold text-gray-900">
                        {messagingService.getOtherParticipant(selectedThread, currentUser?.id)?.username || 'Unknown User'}
                      </h2>
                    </div>
                  </div>
                      ...
                      <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${
                        ...
                        <div className={`flex items-center justify-end mt-1 space-x-1 ${
                          ...
                          <span className="text-xs">
                            {messagingService.formatTimestamp(message.created_at)}
                          </span>
                          {isOwn && (
                            <div className="ml-1">
                              {message.is_read ? (
            ...
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Start New Conversation</h2>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex space-x-3">
                    <button
                      ...
                      disabled={sendingMessage || !newConversation.recipient || !newConversation.content}
